...
function wpg_admin_mail() 
{
  ...
  if (isset($_POST['info_update'])) {
	...
	if ( ! check_email($wpg_options['wp-greet-mail-from']) and $wpg_options['wp-greet-mail-from']!="") {
      echo __('Static From-Address email address is not valid (wrong format or no MX entry for domain).',"wp-greet"). "<br />";
      $upflag=false;
    }

	if ( ! check_email($wpg_options['wp-greet-mail-replyto']) and $wpg_options['wp-greet-mail-replyto']!="") {
...
<div class="wrap">
   ...
   <table class="optiontable">
      ...
      <tr><th scope="row"><?php echo __("Mailtransfermethod","wp-greet")?>:</th>
        <td>
          <input type="radio" name="wp-greet-usesmtp" id="wp-greet-usesmtp1" value="1" <?php if ($wpg_options['wp-greet-usesmtp']=="1") echo "checked=\"checked\" "; ?> onclick="wechsle_smtp();"  />SMTP (class-phpmailer.php)
	      <input type="radio" name="wp-greet-usesmtp" id="wp-greet-usesmtp2" value="0" <?php if ($wpg_options['wp-greet-usesmtp']=="0") echo "checked=\"checked\" "; ?> onclick="wechsle_smtp();" /> PHP mail() function  
	    </td>
	  </tr>
      ...
      <td><input type="checkbox" name="wp-greet-imgattach" id="wp-greet-imgattach" value="1" <?php if ($wpg_options['wp-greet-imgattach']=="1") echo "checked=\"checked\" "; ?> />&nbsp;</td>
	  </tr>
	  <tr class="tr-admin"><td>&nbsp;</td></tr>
