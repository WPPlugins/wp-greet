...
function showGreetcardForm($galleryID, $picurl, $verify = "", $pid = "", $approved = "") {
	...
	if ($verify != "") {
		...
		if ($now > $then and $wpg_options["wp-greet-mcduration"] != 0) {
	...
	if ($wpg_options['wp-greet-future-send'] and isset($_POST['fsend']) and $_POST['fsend'] != "") {
		...
		if ($sendtime < current_time('timestamp',1)) {
			$sendtime = 0;
		}
	}
	else {
	...
	if (!isset($_POST['wpgtitle'])) $_POST['wpgtitle'] = $wpg_options['wp-greet-default-title'];
	// Feldinhalte pruefen
	
	if (isset($_POST['action']) and ($_POST['action'] == __("Preview", "wp-greet") or $_POST['action'] == __("Send", "wp-greet"))) {
		
		if (isset($_POST['sender']) && $_POST['sender'] != '') $_POST['sender'] = esc_attr($_POST['sender']);
		
		if (isset($_POST['sendername']) && $_POST['sendername'] != '') $_POST['sendername'] = stripslashes(esc_attr($_POST['sendername']));
		
		if (isset($_POST['ccsender']) && $_POST['ccsender'] != '') $_POST['ccsender'] = esc_attr($_POST['ccsender']);
		else $_POST['ccsender'] = 0;
		
		if (isset($_POST['wp-greet-enable-confirm']) && $_POST['wp-greet-enable-confirm'] != '') $_POST['wp-greet-enable-confirm'] = esc_attr($_POST['wp-greet-enable-confirm']);
		else $_POST['wp-greet-enable-confirm'] = 0;
		
		if (isset($_POST['accepttou']) && $_POST['accepttou'] != '') $_POST['accepttou'] = esc_attr($_POST['accepttou']);
		else $_POST['accepttou'] = 0;
		
		if (isset($_POST['recv']) && $_POST['recv'] != '') $_POST['recv'] = esc_attr($_POST['recv']);
		
		if (isset($_POST['recvname']) && $_POST['recvname'] != '') $_POST['recvname'] = stripslashes(esc_attr($_POST['recvname']));
		
		if (isset($_POST['wpgtitle']) && $_POST['wpgtitle'] != '') $_POST['wpgtitle'] = esc_attr(stripslashes($_POST['wpgtitle']));
		
		if (isset($_POST['wp-greet-audio']) && $_POST['wp-greet-audio'] != '') $_POST['wp-greet-audio'] = esc_attr(stripslashes($_POST['wp-greet-audio'])); else $_POST['wp-greet-audio']="";
		
		if (isset($_POST['recvbcc']) && $_POST['recvbcc'] != '') $_POST['recvbcc'] = esc_attr($_POST['recvbcc']); else $_POST['recvbcc']="";
		
		if (isset($_POST['wp-greet-offer-stamp']) && $_POST['wp-greet-offer-stamp'] != '') $_POST['wp-greet-offer-stamp'] = esc_attr($_POST['wp-greet-offer-stamp']);
		
		if (isset($_POST['wp-greet-offer-stamp-opacity']) && $_POST['wp-greet-offer-stamp-opacity'] != '') $_POST['wp-greet-offer-stamp-opacity'] = esc_attr($_POST['wp-greet-offer-stamp-opacity']);
		...
		if (($wpg_options['wp-greet-captcha'] > 0) and (isset($_POST['public_key']) or isset($_POST['mcspinfo']) or isset($_POST['cptch_result']) or isset($_POST['recaptcha_challenge_field']))) {
			// check CaptCha!
			...
			if ($wpg_options['wp-greet-captcha'] == 2) {
				...
				if (version_compare($tap['math-comment-spam-protection/math-comment-spam-protection.php']['Version'], "3.0", "<")) $mc_nok = $Cap->InputValidation($_POST['mcspinfo'], $_POST['mcspvalue']);
				else $mc_nok = $Cap->MathCheck_InputValidation($_POST['mcspinfo'], $_POST['mcspvalue']);
			...
			if ($wpg_options['wp-greet-captcha'] == 3) {
				...
				if (class_exists('ReCAPTCHAPlugin')) { // for wp-recaptcha >=4.1
					$reCaptcha = new ReCaptcha($rec_opt['secret']);
	...
	if (isset($_POST['action']) and $_POST['action'] == __("Preview", "wp-greet")) {
		...
		$template.= "<input id='wp-greet-offer-stamp' name='wp-greet-offer-stamp' type='hidden' value='" . $_POST['wp-greet-offer-stamp'] . "' />\n";
		$template.= "<input id='wpg-image-url' name='wpg-image-url' type='hidden' value='" . $_POST['wpg-image-url'] . "' />\n";
		$template.= "<input id='wp-greet-offer-stamp-opacity' name='wp-greet-offer-stamp-opacity' type='hidden' value='" . $_POST['wp-greet-offer-stamp-opacity'] . "' />\n";
		...
		} else if (defined('WCCARDS') and function_exists('is_wp_greet_woocommerce_enabled') and is_wp_greet_woocommerce_enabled() ) {
	...
	if (isset($_POST['action']) and $_POST['action'] == __("Send", "wp-greet") and ($wpg_options['wp-greet-mailconfirm'] != "1" or $verify != "")) {
		// ---------------------------------------------------------------------
		//
		...
		if (checkMailSent($_POST['sender'], $_POST['recv']) == false) {
			...
			if ($wpg_woocommerce_send == true) {
				...
				$autoinc = save_greetcard($_POST['sender'], $_POST['sendername'], $_POST['recv'], $_POST['recvname'], $_POST['wpgtitle'], $_POST['message'], 
										  $wc_picurl, $_POST['ccsender'] * 1 + $_POST['wp-greet-enable-confirm'] * 2, "", // confirm until stays blank
										  $verify, // confirmcode if available
										  get_fetchuntil_date(), uniqid("wpgreet_", false), 3661, // this is 1970-01-01 01:01:01
										  session_id(),$_POST['wp-greet-audio'], $_POST['recvbcc']);
			...
			if ($wpg_options['wp-greet-onlinecard'] == 1) {
				...
				$autoinc = save_greetcard($_POST['sender'], $_POST['sendername'], $_POST['recv'], $_POST['recvname'], $_POST['wpgtitle'], $_POST['message'], $_POST['wpg-image-url'], $_POST['ccsender'] * 1 + $_POST['wp-greet-enable-confirm'] * 2, "", // confirm until stays blank
				$verify, // confirmcode if available
				$fetchuntil, $fetchcode, $sendtime, session_id(),$_POST['wp-greet-audio'], $_POST['recvbcc']);
				//
				...
				if (defined('BWCARDS') and $conn and $bwc_options['bwc_general_formhook'] and $price > 0 and $approved != "approved") {
					...
					if ($sendtime != 0) {
						...
						wp_schedule_single_event($sendtime, "wpgreet_sendcard_link", array(
							$mid,
							$_POST['sender'],
							$_POST['sendername'],
							$_POST['recv'],
				...
				if ($sendtime) {
					...
					wp_schedule_single_event($sendtime, "wpgreet_sendcard_mail", array(
						$mid,
						$_POST['sender'],
						$_POST['sendername'],
						$_POST['recv'],
		...
		if (strval($sendstatus) != "no") { // this is to prevent output if used with bwcards
			...
			if ($sendstatus == true) {
				// show sent message
				if (isset($wpg_options['wp-greet-sent-message'])) {
	...
	if (isset($_POST['action']) and $_POST['action'] == __("Send", "wp-greet") and ($wpg_options['wp-greet-mailconfirm'] == "1" or $verify == "")) {
		...
		if (checkMailSent($_POST['sender'], $_POST['recv']) == false) {
			...
			save_greetcard($_POST['sender'], $_POST['sendername'], $_POST['recv'], $_POST['recvname'], $_POST['wpgtitle'], $_POST['message'], $_POST['wpg-image-url'], $_POST['ccsender'] * 1 + $_POST['wp-greet-enable-confirm'] * 2, $confirmuntil, $confirmcode, "", // fetchuntil stays blank until confirmation
			"", $sendtime,'', $_POST['wp-greet-audio'], $_POST['recvbcc']); // fetchcode stays blank until confirmation
		...
		if ($wpg_options['wp-greet-captcha'] == 3) {
			$cfile = plugin_dir_path(__FILE__) . "../wp-recaptcha/recaptchalib.php";
		...
		if ($captcha != 0) $captcha_label = __("Spamprotection:", "wp-greet");
		// CaptCha!
		...
		if ($captcha == 2) $captcha_input = '<label for="mcspvalue"><small>' . __("Sum of", "wp-greet") . "&nbsp;" . $cap_info['operand1'] . ' + ' . $cap_info['operand2'] . ' ? ' . '</small></label><input type="text" name="mcspvalue" id="mcspvalue" value="" size="23" maxlength="10" /><input type="hidden" name="mcspinfo" value="' . $cap_info['result'] . '" />';
		// WP-reCAPTCHA
		...
		if ($captcha == 3) {
			...
			if (class_exists('ReCAPTCHAPlugin')) { // for wp-recaptcha >=4.1
				$captcha_input = '<div class="g-recaptcha" data-sitekey="' . $rec_opt['site_key'] . '"></div><script type="text/javascript" src="https://www.google.com/recaptcha/api.js?hl=' . $relang . '"></script><br/>';
			}
			else { // older versions
				$rec_opt = get_option('recaptcha_options');
		...
		if ($wpg_options['wp-greet-touswitch'] == 1) {
			...
			$tou_label  = __("I accept the terms of usage of the greeting card service", "wp-greet") . ' <a href="#TB_inline&width=600&height=600&inlineId=wpg-tou" class="thickbox">' . __("(show)", "wp-greet") . "</a>";
			$tou_label .= '<div id="wpg-tou" style="display:none;"><p>'. $wpg_options['wp-greet-termsofusage'] . '</p>p></div>';
